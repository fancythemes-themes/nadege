...
( function( $ ) {
	...
	var style = $( '#nadege-color-scheme-css' ),
		api = wp.customize;

	if ( ! style.length ) {
		style = $( 'head' ).append( '<style type="text/css" id="nadege-color-scheme-css" />' )
		                    .find( '#nadege-color-scheme-css' );
	}
	...
	function writeCSS(){
		...
		$('#nadege-preview-style-inline-css').text(cssOutput);
	}
