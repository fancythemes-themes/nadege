 ...
 * Here is array example: 
	$settings[] = array(
		...
		'control'			=> 'color', 
		'label'				=> __( 'Widget Title Color', 'nadege' ),
		'section'			=> 'colors',
		'priority'			=> 3,
		'apply_css'			=> array (
									array(
 ...
 * Return this array in a function and add_filter to 'nadege_customizer_wrapper_settings' hook
 * Create customizer settings and controls based the arguments
 * with filter 'nadege_customizer_wrapper_settings'
 */
function nadege_customizer_wrapper_settings( $wp_customize ) {

	do_action('nadege_before_customize_wrapper', $wp_customize );

	$settings = array();
	$customize_admin_js_var = array();

	$settings = apply_filters( 'nadege_customizer_wrapper_settings', $settings );
	$i = 1;
	...
	do_action('nadege_after_customize_wrapper', $wp_customize );
}
add_action( 'customize_register', 'nadege_customizer_wrapper_settings', 100, 1 );


/** 
 * Apply the CSS set by the customizer framework based on arguments 
 */
function nadege_apply_customizer_css(){

	$settings = array();
	$settings = apply_filters( 'nadege_apply_customizer_css', $settings );
			...
			foreach ( $setting['apply_css'] as $apply_css ){
				...
				if ( $value && ( $value !== $setting['default'] ) ){
					...
					if ( !empty( $value_in_text ) ) {
						...
						$media_queries[$mq][$selector] .= esc_attr( $property . ': ' . nadege_hex2rgba($value, $color_alpha, true ) . ' ;' );
					} else{
	...
	if ( is_customize_preview() ) {
		wp_enqueue_style( 'nadege-preview-style', get_template_directory_uri() . '/css/customize-preview.css', array( 'nadege-style' ), '20160816' );
		wp_add_inline_style( 'nadege-preview-style', $css );
		//wp_add_inline_style( 'nadege-style', esc_attr($css) );
	} else {
		wp_add_inline_style( 'nadege-style', $css );
	}
	wp_localize_script('nadege-customize-preview', '_customizerCSS', $customize_js_var );

}
add_action('wp_enqueue_scripts', 'nadege_apply_customizer_css', 11 );
 ...
 */
function nadege_hex2rgba( $color, $alpha = 1, $as_css_value = false ) {
	$color = trim( $color, '#' );
 ...
 */
function nadege_customize_preview_js() {
	wp_enqueue_script( 'nadege-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20130508', true );
}
add_action( 'customize_preview_init', 'nadege_customize_preview_js' );

/**
 * Binds JS handlers for the helper in the customizer admin.
 */
function nadege_customize_admin_js() {
	wp_enqueue_script( 'nadege-customizer-admin', get_template_directory_uri() . '/js/customize-control.js', array( ), '20141114', true );
}
add_action( 'customize_controls_enqueue_scripts', 'nadege_customize_admin_js' );